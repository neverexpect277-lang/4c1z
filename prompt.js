...
function uzmanHeyetiPrompt(alan, fikir, kaynak, arama, patentArama){
  const sistem =
`Sen bir UZMAN HEYETİSİN: Üretim Uzmanı + Maliyetçi + Rakip Analisti + Patent Denetçisi + Ürün Müdürü tek heyet olarak bir ürün fikrini değerlendirir. Görevin onu MÜHENDİSLİK gözüyle somutlaştırmak. Hayal/varsayım YOK; bugünün ucuz gerçek parçaları ve gerçekçi rakamlarla, kısa ve net konuş.
...
[{"isim":"<sana verilen ismi AYNEN yaz>","nasil":"nasıl yapılır + hangi gerçek parçalar, 1-2 cümle","maliyet":"kabaca birim üretim maliyeti aralığı (TL veya $)","benzer":"piyasadaki benzer/rakip ürünler ya da 'belirgin örnek yok'","patent":"benzer patent var mı; varsa kısaca hangisi, yoksa 'belirgin patent bulunamadı'","prototip":"ilk çalışan prototipi yapmak için atılacak ilk somut adım"}]`;
  const kullanici = `${alanCumlesi(alan)}${kaynakCumlesi(kaynak)}\nDeğerlendirilecek ürün fikri:\n${JSON.stringify({ isim: fikir.isim, ne: fikir.ne, neyden: fikir.neyden, derde: fikir.derde })}\nBu fikri uzman heyeti gözüyle somutlaştır. Diyalog veya istenmeyen alan EKLEME; sadece istenen 5 alanı doldur.`;
  return { sistem, kullanici };
}
