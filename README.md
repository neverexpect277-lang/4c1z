...
## Özellikler
Kaydet (★) + durum/puan/not · alan filtresi + metin arama · isimli temalar · "Önerilen Fikirler" katlanabilir kutusu · kaynak alanı (NotebookLM mantığı) · **canlı ajan zinciri + öğrenen hafıza** (mastra ilhamı: 4 aşama ekranda adım adım ilerler; beğenilen kayıtlar üreticiye pozitif sinyal olur, geçmiş fikirler tekrarlanmaz) · **ayarlanabilir akış motoru** (dify ilhamı: 4 aşamayı düzenlenebilir akış olarak gör; aday sayısı, eleme, diyalog tonu, web araması düğümden ayarlanır, ayarlar kalıcı) · PWA (telefona kurulur) · kalıcı oturum (localStorage).

## Teknik
Saf vanilla JS + Vercel serverless (`api/gen` Gemini, `api/poll` Pollinations, `api/ara` çok-kaynaklı arama). Backend/veritabanı yok. 170 jsdom testi (`npm test`).
