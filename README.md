...
## Özellikler
Kaydet (★) + durum/puan/not · alan filtresi + metin arama · isimli temalar · "Önerilen Fikirler" katlanabilir kutusu · kaynak alanı (NotebookLM mantığı) · **düğüm tabanlı görsel üretimi** (ComfyUI ilhamı: Konu→Stil→Arka plan→Üret düğümleri tek prompt'a zincirlenir, `/api/image` flux ile ürün görseli üretir) · PWA (telefona kurulur) · kalıcı oturum (localStorage).

## Teknik
Saf vanilla JS + Vercel serverless (`api/gen` Gemini, `api/poll` Pollinations, `api/ara` çok-kaynaklı arama, `api/image`). Backend/veritabanı yok. 160 jsdom testi (`npm test`).
