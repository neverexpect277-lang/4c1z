...
Web arama (SearXNG + DuckDuckGo + Wikipedia tr/en) · GitHub (★/fork/topics) · Stack Exchange · Hacker News · Reddit (gerçek kullanıcı dert/talep sinyali) · Google Patents · arXiv · Wikidata (kavram varlığı) · Semantic Scholar (geniş bilimsel) · npm (yazılım sinyali) · Open Library (kavram olgunluğu) · Datamuse kelime genişletme · Frankfurter güncel USD→TRY kuru. İngilizce arama perde arkasında çalışır, çıktı %100 Türkçe.
...
## Teknik
Saf vanilla JS + Vercel serverless (`api/gen` Gemini, `api/poll` Pollinations, `api/ara` çok-kaynaklı arama, `api/cek` URL→metin). Backend/veritabanı yok. 239 jsdom testi (`npm test`).
