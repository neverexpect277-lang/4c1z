...
## Özellikler
Kaydet (★) + durum/puan/not · alan filtresi + metin arama · isimli temalar · **mod kalıpları** (fabric ilhamı: ucuz prototip / yüksek teknoloji / sosyal fayda / çevre / eğlence — tek tıkla üretimi o tarza yönlendirir) · "Önerilen Fikirler" katlanabilir kutusu · kaynak alanı (NotebookLM mantığı) · **URL'den kaynak çekme** (firecrawl ilhamı: kaynak kutusuna link yapıştır → "Linki çek" → sayfa sunucudan temiz metne dönüşür, `/api/cek`, anahtarsız) · **akıllı kaynak seçimi** (ragflow ilhamı: uzun belge cümlelere bölünür, alanla en alakalı kısımlar seçilip beslenir — belge promptu boğmaz, anahtarsız RAG) · **canlı ajan zinciri + öğrenen hafıza** (mastra ilhamı: 4 aşama ekranda adım adım ilerler; beğenilen kayıtlar üreticiye pozitif sinyal olur, geçmiş fikirler tekrarlanmaz) · **ayarlanabilir akış motoru** (dify ilhamı: 4 aşamayı düzenlenebilir akış olarak gör; aday sayısı, eleme, diyalog tonu, web araması düğümden ayarlanır; ayrıca otomasyon: toplu üretim + skor tetikli oto-kaydet — n8n ilhamı; ayarlar kalıcı) · PWA (telefona kurulur) · **kartta canlı pazar taraması** (browser-use ilhamı: "Pazarı tara" → fikre özel web/rakip taraması, /api/ara ajan ordusu) · kalıcı oturum (localStorage).

## Teknik
Saf vanilla JS + Vercel serverless (`api/gen` Gemini, `api/poll` Pollinations, `api/ara` çok-kaynaklı arama, `api/cek` URL→metin). Backend/veritabanı yok. 201 jsdom testi (`npm test`).
