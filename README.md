...
## Özellikler
Kaydet (★) + durum/puan/not · alan filtresi + metin arama · isimli temalar · "Önerilen Fikirler" katlanabilir kutusu · kaynak alanı (NotebookLM mantığı) · PWA (telefona kurulur) · kalıcı oturum (localStorage).

## Teknik
Saf vanilla JS + Vercel serverless (`api/gen` Gemini, `api/poll` Pollinations, `api/ara` çok-kaynaklı arama, `api/image`). Backend/veritabanı yok. 143 jsdom testi (`npm test`).
